...
class PresentationTool():
    def _run(self, topic: str,num_slides: int, theme: str) -> str:
        try:
            
            api_url=os.getenv('AWS_URL_PPT')
            payload = {
                "topic": topic,
            ...
            with requests.post(api_url, json=payload, stream=True) as response:
                ...
                filename = f"{payload['topic'].replace(' ', '_')}_presentation.pptx"
                files = {
                    "file": (filename, content, "application/vnd.openxmlformats-officedocument.presentationml.presentation")
                }
                print(files)
                ...
                return f"Presentation created and uploaded. Download link: {download_link}"
